...
GEGVICshine can be used through VHIO's server. However, the computational power of this server is limited. For this reason, analyses involving more than 50 samples are discouraged. In case more power is required, we recommend using GEGVICshine locally. 
...
**All files must be .csv files (comma separated values) except for the gene sets that must be in the form of a .gmt file.**
...
- *To use the CIBERSORT algorithm, the user need to register on the CIBERSORT web page (https://cibersort.st anford.edu), obtain a license and download the source code in form of two files CIBERSORT.R and LM22.txt. Then both files need to be uploaded in the corresponding space.*
